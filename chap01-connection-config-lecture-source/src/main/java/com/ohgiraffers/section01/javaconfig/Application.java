...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        /* 필기
         *  SqlSessionFactory: SqlSession 객체를 생성하기 위한 팩터리 역할의 인터페이스
         *  SqlSessionFactoryBuilder: SqlSessionFactory 인터페이스 타입의 하위 구현 객체를 생성하기 위한 빌드 역할
         *  build(): 설정에 대한 정보를 담고 있는
         *           Configuration 타입의 객체 혹은 외부 설정 파일과 연결된 stream 매개변수로 전달하면
         *           SqlSessionFactory 인터페이스 타입의 객체를 반환하는 메소드
        * */
        SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(configuration);
        SqlSession session = sqlSessionFactory.openSession(false);      // 서비스 계층에서 하는 일이 될 것임. (아직 계층 안나눔)
